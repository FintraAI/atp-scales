...
const schema = z.object({
  ...
  status:        z.enum(['ACTIVE', 'PAUSED', 'CHURNED', 'ONBOARDING', 'PROSPECT']).default('ONBOARDING'),
  // Onboarding quick-fill (all optional)
...
export async function POST(req: NextRequest) {
  ...
  const {
    email, name, password, companyName, industry, website,
    phone, address, monthlyBudget, planName, status,
    offerName, targetAudience, callNotes,
  } = parsed.data
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    // Create onboarding record if any fields provided
