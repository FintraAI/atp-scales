            ...
            <div className="md:col-span-2 space-y-6">
              {[
                ...
                <div key={label} className="flex items-start gap-4">
                  <div className="w-9 h-9 rounded-lg bg-[rgba(201,168,76,0.08)] border border-[rgba(201,168,76,0.15)] flex items-center justify-center shrink-0">
                    <Icon className="w-4 h-4 text-[#C9A84C]" />
                  </div>
                  <div>
                    <p className="text-[11px] uppercase tracking-wider font-bold text-[#444] mb-0.5">{label}</p>
                    <p className="text-[#888] text-sm">{value}</p>
                  </div>
                </div>
              ))}

              {/* What to expect */}
              <div className="mt-8 p-5 bg-[#111] border border-[#1C1C1C] rounded-2xl">
    ...
    <div className="bg-[#111] border border-[#1C1C1C] rounded-2xl p-7">
      <form className="space-y-4" action="/api/contact" method="POST">
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-[11px] font-bold uppercase tracking-wider text-[#444] mb-1.5">
              First Name
            </label>
            <input
              name="firstName"
