    ...
    <div className="relative rounded-2xl overflow-hidden">
      ...
      <div className="absolute inset-0 flex items-center justify-center bg-[#0A0A0A]/60 backdrop-blur-[1px]">
        <div className="atp-card p-6 text-center max-w-sm mx-4 shadow-2xl">
          <div className="w-10 h-10 rounded-xl bg-[rgba(212,175,55,0.08)] border border-[rgba(212,175,55,0.15)] flex items-center justify-center mx-auto mb-3">
            <Lock className="w-4 h-4 text-[#D4AF37]" />
          </div>
          <p className="font-display font-bold text-[#FFFFFF] tracking-wide mb-1">
            Available on {requiredPlan}
          </p>
          <p className="text-[#6B6B6B] text-[12px] leading-relaxed mb-4">
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-end justify-between">
        <span className="text-[11px] font-bold uppercase tracking-[0.1em] text-[#6B6B6B]">{label}</span>
        <div className="text-right">
          <span className="font-display font-bold text-[20px] text-[#FFFFFF]">{formatNumber(value, true)}</span>
          <span className="text-[10px] text-[#333] ml-1.5">{sub}</span>
        </div>
      </div>
      <div className="h-2 bg-[#1A1A1A] rounded-full overflow-hidden">
        <div
          className="h-full rounded-full bg-gradient-to-r from-[#8A6E2F] to-[#D4AF37] transition-all duration-700"
          style={{ width: `${Math.max(pct, 2)}%` }}
        />
      </div>
      <p className="text-[11px] text-[#6B6B6B]">{pct.toFixed(1)}% of total</p>
    </div>
  )
}
    ...
    <div className="atp-card p-6">
      <div className="h-[180px] flex items-end gap-1 px-4 pb-4">
        {[40, 65, 45, 80, 55, 90, 70, 85, 60, 95, 75, 88].map((h, i) => (
          <div key={i} className="flex-1 rounded-t-sm" style={{ height: `${h}%`, background: 'rgba(212,175,55,0.15)' }} />
        ))}
      </div>
    </div>
  )
...
function FakeTablePlaceholder() {
  return (
    <div className="atp-card overflow-hidden">
      ...
      {[1, 2, 3].map(i => (
        <div key={i} className="px-6 py-4 border-b border-[#0F0F0F] flex items-center justify-between">
...
export default async function AnalyticsPage() {
  ...
  const PLAN_BADGE: Record<PlanName, { label: string; color: string }> = {
    Starter: { label: 'Starter',  color: 'text-[#B3B3B3]  bg-[#1A1A1A] border-[#2A2A2A]' },
    Growth:  { label: 'Growth',   color: 'text-blue-400 bg-blue-400/8 border-blue-400/20' },
    Scale:   { label: 'Scale',    color: 'gold-text bg-[rgba(212,175,55,0.06)] border-[rgba(212,175,55,0.18)]' },
  }
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="font-display text-2xl font-bold text-[#FFFFFF] tracking-wide">Analytics</h1>
          <p className="text-[#6B6B6B] text-sm mt-1">Last 30 days · performance deep-dive</p>
        </div>
      ...
      <section className="space-y-4">
        <h2 className="font-display font-bold text-[#FFFFFF] tracking-wide">30-Day Summary</h2>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <div key={s.label} className={s.gold ? 'atp-card-gold kpi-card' : 'kpi-card'}>
              ...
              <p className={`font-display font-bold text-xl leading-none ${s.gold ? 'gold-text' : 'text-[#FFFFFF]'}`}>
                {s.value}
              </p>
            </div>
          ))}
      ...
      <section className="space-y-4">
        <h2 className="font-display font-bold text-[#FFFFFF] tracking-wide">Performance Over Time</h2>
              ...
              <table className="w-full data-table">
                ...
                <tbody>
                  {campaignRows.map(c => (
                    <tr key={c.id}>
                      <td>
                        <p className="font-semibold text-[#FFFFFF] text-[13px] max-w-[220px] truncate">{c.name}</p>
      ...
      {planAtLeast(plan, 'Growth') && (
        <section className="space-y-4">
          <h2 className="font-display font-bold text-[#FFFFFF] tracking-wide">90-Day Trend</h2>
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-3">
          <h2 className="font-display font-bold text-[#FFFFFF] tracking-wide">Full Funnel Analysis</h2>
        ...
        {planAtLeast(plan, 'Scale') ? (
          <div className="atp-card p-6 space-y-1">
            <p className="text-[11px] text-[#6B6B6B] font-bold uppercase tracking-[0.12em] mb-6">
            ...
            <div className="mt-8 pt-6 border-t border-[#222222] grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                ...
                <div key={m.label}>
                  <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#6B6B6B] mb-1">{m.label}</p>
              ...
              <div className="max-w-lg space-y-4">
                {['Impressions', 'Clicks', 'Leads', 'Appointments', 'Conversions'].map((s, i) => (
                  <div key={s} className="space-y-1.5">
                    ...
                    <div className="h-2 bg-[#1A1A1A] rounded-full overflow-hidden">
                      <div className="h-full rounded-full bg-gradient-to-r from-[#8A6E2F] to-[#D4AF37]"
